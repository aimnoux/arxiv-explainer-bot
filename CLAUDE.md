...
**Команды бота:** `/start` — приветствие + ссылка на репозиторий https://github.com/aimnoux/arxiv-explainer-bot. `/help` — подробная инструкция по использованию (форматы ссылок, описание 7 разделов разбора). `/admin` — только для владельца. Команды `/myid` и `/status` удалены.
