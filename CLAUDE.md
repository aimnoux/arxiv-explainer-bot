...
```bash
# Деплой на VPS (всё включено: venv, зависимости, wizard, systemd, CLI-команда)
sudo ./setup.sh

# Панель управления (после деплоя)
arxiv-bot

# Локальный запуск без деплоя
python3 -m venv .venv && source .venv/bin/activate
pip install -r requirements.txt
python3 -m bot.wizard   # настройка
...
├── wizard.py        # интерактивный мастер настройки (только stdlib)
└── cli.py           # постоянная CLI-панель управления (команда arxiv-bot)
```
