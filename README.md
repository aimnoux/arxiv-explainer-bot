...
╚══════════════════════════════════════╝
  ...
  [5] Установить обновления  (сначала [4])

  [0] Выход
```

Если обновления найдены, пункт `[5]` подсвечивается жёлтым:

```
  [5] ✨ Установить обновления        ← ДОСТУПНО
```

После установки обновлений панель перезапускается автоматически.
...
Через панель управления: `arxiv-bot` → `[2] Настроить (wizard)`. Панель сама остановит и перезапустит бота.
...
│   └── cli.py           # панель управления (команда arxiv-bot)
