...
|---------|--------|----------|
| `/start` | все | Приветствие и ссылка на репозиторий |
| `/help` | все | Как пользоваться ботом: форматы ссылок и что входит в разбор статьи |
| `/admin` | владелец | Панель настройки для владельца: провайдер, модель, API-ключ и язык |
